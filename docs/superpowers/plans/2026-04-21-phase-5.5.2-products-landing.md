...
- [ ] All 25 tasks above complete, every commit landed.
- [ ] `pnpm exec tsc --noEmit` passes.
- [ ] `pnpm test` passes with ≥ baseline test count (+ new tests from Wave 1).
- [ ] `pnpm build` passes; all new routes listed.
- [ ] Admin can create / edit / archive / restore / reorder products at `/admin/catalog/products`.
- [ ] Landing page at `/` renders a branded product grid; zero-product tenants fall through to `/configurator`.
- [ ] Configurator at `/configurator?product=<slug>` spawns a building with product defaults; `sourceProductId` lands on the building.
- [ ] Material pickers narrow by product constraints; dimension sliders clamp to product min/max; "Beperkt door bouwset" helper renders when constraints narrow a slot.
- [ ] Tray shows Bouwsets + Losse elementen; `overkapping` / `berging` tray buttons removed (products replaced them).
- [ ] "Bouwset" chip + "Herstel standaarden" action visible on product-sourced buildings.
- [ ] Assymo seeded with two example products (Standaard Overkapping 4×3, Standaard Berging 3×3).
- [ ] CLAUDE.md reflects products + landing + /configurator move.
